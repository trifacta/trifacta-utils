...
function backup_dbs
{
	...
	su -l postgres -c "${PG_DUMP} ${webapp_db_name} > ${webapp_db_backup}"
	if [ $? -eq 0 ]; then
	...
	su -l postgres -c "${PG_DUMP} ${scheduling_service_db_name} > ${scheduling_service_db_backup}"
	if [ $? -eq 0 ]; then
	...
	su -l postgres -c "${PG_DUMP} ${time_based_trigger_service_db_name} > ${time_based_trigger_service_db_backup}"
	if [ $? -eq 0 ]; then
